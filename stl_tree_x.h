#ifndef _STL_TREE_X_H
#define _STL_TREE_X_H
...
namespace _LXX
{
	...
	struct __rb_tree_iterator : public __rb_tree_iterator_base
	{
		...
		__rb_tree_iterator(){ node = 0; }
		__rb_tree_iterator(link_type p){ node = p; }
	...
	template<class Key, class Value, class KeyOfValue,class Compare,class Alloc = _LXX::default_alloc>
	class rb_tree
	{
	protected:
		typedef __rb_tree_node_base * base_ptr;
	...
	protected:
		link_type get_node(){ return (link_type) rb_tree_node_allocator::allocate(); }
	...
	template<class Iter>
	bool equal(Iter xfirst, Iter xend, Iter yfirst)
	{
		while (xfirst != xend)
		{
			if (*xfirst != *yfirst)
	...
	template<class Key, class Value, class KeyOfValue, class Compare, class Alloc>
	inline void swap(const rb_tree<Key, Value, KeyOfValue, Compare, Alloc> & x, const rb_tree<Key, Value, KeyOfValue, Compare, Alloc> & y)
	{
		x.swap(y);
	}
